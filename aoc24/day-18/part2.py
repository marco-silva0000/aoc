    ...
    while len(open_list) > 0:
        ...
        for neighbour, direction in n.tile.location.get_orthogonal_neighbours():
            if (
                ...
                and grid[neighbour].tile_type != TileType.CORRUPTED
...
def part2(values_list) -> str:
    ...
    max_y = 7
    ticks = 1200000
    for i, byte in enumerate(values_list):
    ...
    for t in range(ticks):
        log.info("tick", tick=t)
        try:
            byte = byte_list[t]
        except IndexError:
            break
        grid[byte.location] = byte
        ...
        try:
            ...
            print(byte.location)
            print(byte.location)
            return f"{byte.location.x},{byte.location.y}"
