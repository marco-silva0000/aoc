...
def part2(values_list) -> str:
    ...
    log = get_logger()
    log.info("day 15 part2")
    grid: Dict[Point, Tile] = dict()
    grid_expanded: Dict[Point, Tile] = dict()
    grid_sm: Dict[Point, Tile] = dict()
    in_directions = False
    directions = []
    for y, line in enumerate(values_list):
        ...
        for x, c in enumerate(line):
            if not in_directions:
                ...
                match tile_type:
                    case TileType.EMPTY:
                        grid_expanded[p1] = Tile(TileType.EMPTY, p1)
                        grid_expanded[p2] = Tile(TileType.EMPTY, p2)
                    case TileType.ROBOT:
                        grid_expanded[p1] = Tile(TileType.ROBOT, p1)
                        grid_expanded[p2] = Tile(TileType.EMPTY, p2)
                        start = p1
                    case TileType.BOX:
                        grid_expanded[p1] = Tile(TileType.BOX_L, p1)
                        grid_expanded[p2] = Tile(TileType.BOX_R, p2)
                    case TileType.WALL:
                        grid_expanded[p1] = Tile(TileType.WALL, p1)
                        grid_expanded[p2] = Tile(TileType.WALL, p2)

            if in_directions:
                direction = Direction.from_arrows(c)
                directions.append(direction)

    max_x = len(values_list[0]) * 2
    min_x = 0
    min_y = 0
    grid_sm = grid
    grid = grid_expanded
    print_grid(grid_sm, max_x // 2, max_y)
    print_grid(grid_expanded, max_x, max_y)
    ...
    def can_move(point, direction):
        ...
        match (neighbour_tile := grid[neighbour]).tile_type:
            ...
            case TileType.WALL:
                ...
                return False
            case TileType.BOX | TileType.BOX_L | TileType.BOX_R:
                logger.debug(
                    "maybe can, is box",
                    point=point,
                    direction=direction,
                ...
                if direction in [Direction.NORTH, Direction.SOUTH]:
                    if neighbour_tile.tile_type == TileType.BOX_L:
                        l_box_location = neighbour_tile.location
                        r_box_location = neighbour_tile.location + Point(1, 0)
                        logger.debug(
                            "box is L",
                            direction=direction,
                            l_box_location=l_box_location,
                            r_box_location=r_box_location,
                        )
                        return all(
                            (
                                can_move(l_box_location, direction),
                                can_move(r_box_location, direction),
                            )
                        )
                    else:
                        return all(
                            (
                                can_move(
                                    neighbour_tile.location + Point(-1, 0), direction
                                ),
                                can_move(neighbour_tile.location, direction),
                            )
                        )
                else:
    ...
    def find_moves(point, direction):
        ...
        match (neighbour_tile := grid[neighbour]).tile_type:
            ...
            case TileType.BOX | TileType.BOX_L | TileType.BOX_R:
                if direction not in (Direction.NORTH, Direction.SOUTH):
                    ...
                    if neighbour_tile.tile_type == TileType.BOX_L:
                        return (
                            find_moves(neighbour_tile.location, direction)
                            + find_moves(
                                neighbour_tile.location + Point(1, 0), direction
                            )
                            + [(point, neighbour)]
    ...
    for index, direction in enumerate(directions):
        ...
        if can_move(current, direction):
            ...
            moves = list(dict.fromkeys(moves))  # remove duplicates preserving order
            for move in moves:
    ...
    for y in range(max_y):
        for x in range(max_x):
            point = Point(x, y)
            if (tile := grid[point]).tile_type == TileType.BOX_L:
