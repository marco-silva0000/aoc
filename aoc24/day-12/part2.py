...
def calc_sides(
    ...
    def filter_valid_sides_maker(position):
        position_value = grid[position].value

        def filter_valid_sides(p):
            logger.debug(f"filtering for {position}", p=p)
            try:
                logger.debug(
                    "trying",
                    position_value=position_value,
                    p_value=grid[p[0]].value,
                )
                return grid[p[0]].value != position_value
            except KeyError:
                logger.debug("key error")
                return True

        return filter_valid_sides
...
def calc_sides(
    ...
    min_x = min([pos.x for pos in region])
    min_y = min([pos.y for pos in region])
    max_x = max([pos.x for pos in region])
    max_y = max([pos.y for pos in region])
    logger.debug(
        f"calculating for region",
        min_x=min_x,
        min_y=min_y,
        max_x=max_x,
        max_y=max_y,
    ...
    for y in range(min_y - 1, max_y + 2):
        ...
        for x in range(min_x - 1, max_x + 2):
            ...
            current = point.x
            try:
                logger.debug("trying", point=point)
                if point in region:
                    logger.debug("in region", point=point)
                    if not is_in:
                        logger.debug("in now in", point=point)
                        in_at.append(current)
                        is_in = True
                        if current not in prev_in_at:
                            logger.debug("appending is in")
                            result += 1
                elif is_in:
                    logger.debug("in now out", point=point)
                    out_at.append(current)
                    is_in = False
                    if current not in prev_out_at:
                        logger.debug("appending is out")
                        result += 1
            except KeyError:
                logger.debug("key_error", point=point)
                continue
...
def part2(values_list) -> str:
    ...
    for region in regions:
        ...
        sides = calc_sides(region[1], grid)
        area = calc_area(region[1])
        log.debug(
            f"calculating for region {region[0]}, sides: {sides} * {area} :area = {sides*area}"
        )
        result += sides * area
    print(result)
    print(result)
