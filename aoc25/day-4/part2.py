...
def part2(values_list) -> str:
    ...
    while True:
        ...
        for point, tile in grid.items():
            if tile.tile_type == TileType.PAPER:
                log = log.bind(x=point.x, y=point.y, tile=tile.tile_type)
                paper_neighbours = 0
                for neighbour, _ in point.get_all_neighbours():
                    if (
                        in_bounds(
                            neighbour,
                            min_x,
                        ...
                        and grid[neighbour].tile_type == TileType.PAPER
                    ):
                        paper_neighbours += 1
                if paper_neighbours < 4:
                    log.debug("appended to forkliftable", forkliftable=forkliftable)
                    forkliftable.append(point)

        log.debug("will remove forkliftable", forkliftable=forkliftable)
        for point in forkliftable:
            node = grid[point]
            node.tile_type = TileType.REMOVED
            grid[point] = node
        if len(forkliftable) == 0:
            break

    result = sum(
        1 for _ in filter(lambda x: x.tile_type == TileType.REMOVED, grid.values())
    )
    print(result)
    return f"{result}"
