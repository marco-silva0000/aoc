...
def print_world(world, min_x, max_x, min_y, max_y, marked_x_for_expansion=None, marked_y_for_expansion=None):
    print(f"  ", end="")
    ...
    for y in range(min_y, max_y + 1):
        for x in range(min_x, max_x + 1):
            if x == min_x:
                print(f"{y % 10}:", end="")
            if (marked_x_for_expansion and x in marked_x_for_expansion) or (marked_y_for_expansion and y in marked_y_for_expansion):
                print("⚫", end="")
...
def part2(values_list, expansion=1) -> str:
    ...
    print_world(galaxies, 0, length, 0, height, marked_x_for_expansion, marked_y_for_expansion)
    ...
    for g1 in galaxies.keys():
        for g2 in galaxies.keys():
            ...
            distance = abs(g1[0] - (g2[0] )) + x_between*expansion + abs(g1[1] - (g2[1] ))+ y_between*expansion
