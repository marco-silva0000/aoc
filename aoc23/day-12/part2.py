...
@cached(
    cache={},
    key=lambda permutation, group, group_index=0, current_group=0, i=None, carry_over="": hashkey(
        permutation, group, group_index, current_group
    ),
)
def validate_permutation(
    permutation, group, group_index=0, current_group=0, i=None, carry_over=""
):
    ...
    if group_index >= len(group):
        ...
        return carry_over + permutation.replace(
            Spring.UNKNOWN.value, Spring.OPERATIONAL.value
        )
    ...
    if current_group > group_value:
        ...
        if permutation.startswith(Spring.OPERATIONAL.value):
            return validate_permutation(
                ...
                current_group=0,
                i=i,
                carry_over=carry_over + Spring.OPERATIONAL.value,
            )
    ...
    if permutation.startswith(Spring.OPERATIONAL.value):
        ...
        if current_group == group_value:
            ...
            return validate_permutation(
                ...
                i=i,
                carry_over=carry_over + Spring.DAMAGED.value,
            )
...
def validate_permutation(permutation, groups):
    ...
    def is_damaged():
        ...
        if (
            slice.count(Spring.DAMAGED.value) + slice.count(Spring.UNKNOWN.value)
            != current_group
        ):
            return 0
        if len(permutation) == current_group:
            if len(groups) == 1:
                ...
                return 0
        if permutation[current_group] in [
            Spring.OPERATIONAL.value,
            Spring.UNKNOWN.value,
        ]:
