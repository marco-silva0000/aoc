...
def part1(values_list) -> str:
    ...
    while len(unprocessed_beam_list) > 0:
        ...
        grid[beam.position].is_energized = True
        match beam.direction:
            case Direction.NORTH:
                if grid[beam.position].has_been_north:
                    continue
                grid[beam.position].has_been_north = True
            case Direction.SOUTH:
                if grid[beam.position].has_been_south:
                    continue
                grid[beam.position].has_been_south = True
            case Direction.EAST:
                if grid[beam.position].has_been_east:
                    continue
                grid[beam.position].has_been_east = True
            case Direction.WEST:
                if grid[beam.position].has_been_west:
                    continue
                grid[beam.position].has_been_west = True
        # logger.debug(beam)
        tile = grid[beam.position]
    ...
    result = [tile.is_energized for tile in grid.values()].count(True)
