...
def get_reflection(pattern, rotate=False):
    ...
    for i in range(1, len(pattern)):
        ...
        for l, r in zippy:
            zippy2 = zip(l, r)
            zippy2_list = list(zippy2)
            logger.debug(f"in zippy{diffs}", l=l, r=r, zippy2=zippy2_list)
            for bl, br in zippy2_list:
                logger.debug("in zippy2", bl=bl, br=br, diffs=diffs)
                if diffs > 1:
        ...
        if diffs == 1:
            logger.debug("real reflection :)", diffs=diffs)
            return i
    return 0
