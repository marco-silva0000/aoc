...
class Mapy:
        ...
        for (this_start, next_start, length) in self.ranges:
            if start >= this_start and start < this_start + length:
                ...
                if end <= this_start + length:
                    ...
                    result.append(
                        (
                            next_start + new_start_diff,
                            next_start + new_start_diff + end_diff,
                        )
                    )
                    ...
                    result.append(
                        (start + next_start_diff, this_start + length + next_start_diff)
                    )
    ...
    def propagate_ranges(
        self, min_max: List[int], target: str
    ) -> List[Tuple[int, int]]:
            ...
            if this_start != prev_start + prev_length:
                ranges_copy.insert(
                    ...
                    (
                        ...
                        this_start,
                        this_start - (prev_start + prev_length),
                    ),
                )
            prev = r
